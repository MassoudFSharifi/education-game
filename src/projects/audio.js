  ...
  useEffect(() => {
    if (ref.current === false) {
      if (WebGL.isWebGLAvailable()) {
        ...
        const addMaterial = (model, color, map) => {
          const material = new THREE.MeshStandardMaterial({
            color: color ? color : map ? undefined : 0xfafafa,
            map,
          });
        ...
        assetLoader.load("./3d/avali-circle-table/scene.gltf", (gltf) => {
          ...
          addMaterial(
            circleTable,
            undefined,
            textureLoader.load("./assets/images/booth/circle-table.jpg")
          );
        });
        ...
        audioLoader.load("assets/audio/bbb.mp3", (buffer) => {
